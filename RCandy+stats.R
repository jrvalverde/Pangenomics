...
# Save all output
if (verbose) {
    logfile="R.RCandy.log"
} else {
    logfile="R.RCandy.sum"
}
...
ref.genome.gff.file <- "zygREF-Ec-ST131-no-plasmids.gff3"
...
if ( ! file.exists("R.gubbins.GFF.Rds") ) {
    gubbins.GFF <- load.gubbins.GFF(gubbins.gff.file, recom.input.type = "Gubbins") 
    saveRDS(gubbins.GFF, "R.gubbins.GFF.Rds")
    # cannot be saved as tab-separated
} else {
    gubbins.GFF <- readRDS("R.gubbins.GFF.Rds")
}

cat("Reading recombination events\n")
if ( ! file.exists("R.rec.events.Rds") ) {
    #rec.events <- load.gubbins.GFF(gubbins.gff.file, recom.input.type = "Gubbins")
    rec.events <- gubbins.GFF
    saveRDS(rec.events, file="R.rec.events.Rds")
    # load with rec.events <- readRDS("R.rec.events.Rds")
} else {
    rec.events <- readRDS("R.rec.events.Rds")
    # cannot be saved as tab-separated
}

cat("Reading phylogenetic tree\n")
if (!  file.exists("R.tree.Rds") ) {
    #tree <- ape::read.tree.file(tree.file)
    tree <- read.tree.file(tree.file)
    saveRDS(tree, "R.tree.Rds")
} else {
    tree <- readRDS("R.tree.Rds")
    # cannot be saved as tab-separated
}

cat("Reading metadata\n")
if ( ! file.exists("R.metadata.df.Rds") ) {
    metadata.df <- load.taxon.metadata(metadata.file)
    saveRDS(metadata.df, "R.metadata.df.Rds")
    # cannot be saved as tab-separated
} else {
    metadata.df <- readRDS("R.metadata.df.Rds")
}
metadata <- tidyr::as_tibble(read.table(metadata.file, header=T, sep='\t', comment.char="?"))

cat("Reading reference genome\n")
if ( ! file.exists("R.refgenome.gff.Rds") ) {
    refgenome.GFF <- load.genome.GFF(ref.genome.gff.file)
    saveRDS(refgenome.GFF, "R.refgenome.GFF.Rds")
    # cannot be saved as tab-separated
} else {
    refgenome.GFF <- readRDS("R.refgenome.GFF.Rds")
}

cat("Reading rec frequency per base\n")
if ( ! file.exists("R.rec.freq.Rds") ) {
    rec.freq <- count.rec.events.per.base(gubbins.gff.file, recom.input.type="Gubbins")
    saveRDS(rec.freq, file="R.rec.freq.Rds")
    write.table(rec.freq, file="R.rec.freq.tab", sep='\t', row.names=FALSE)
} else {
    rec.freq <- readRDS("R.rec.freq.Rds")
}

cat("Reading rec events per genome\n")
if ( ! file.exists("R.rec.genome.Rds") ) {
    ...
    saveRDS(rec.genome, file="R.rec.genome.Rds")
    write.table(rec.genome, file="R.rec.genome.tab", sep='\t', row.names=FALSE)
    head(rec.genome)
} else {
    rec.genome <- readRDS("R.rec.genome.Rds")
}
...
make.backup("R.RCandy_plots.pdf")
pdf("R.RCandy_plots.pdf", paper="a4", width=8, height=11)
...
if ( ! file.exists("R.RCandy_plot.pdf") ) {
    ...
    RCandyVis(
        ...
        save.to.this.file="R.RCandy_plot.pdf"
...
RCandyVis(
    ...
    rec.events.per.base.as.heatmap=FALSEE,
    rec.heatmap.color=c("#FF000022", "#0000FF66"),
    rec.plot.bg.transparency=0.0,
    show.fig.legend=TRUE,
    save.to.this.file=NULL
...
if ( ! file.exists("R.RCandy_gyrA_region.pdf") ) {
    ...
    RCandyVis(tree.file.name = tree, 
        ...
        taxon.metadata.columns = c("S83L", "D87N", "S83L.D87N"), 
        taxon.id.column = "ID", 
        genome.start = 1030000, genome.end = 1040000, 
        ...
        #trait.for.ancestral.reconstr='S83L',
        save.to.this.file = "R.RCandy_gyrA_region.pdf"
    )
}
...
if (! file.exists("R.RCandy_parC_region.pdf") ) {
    ...
    RCandyVis(tree.file.name = tree, 
        ...
        rec.events.per.base.as.heatmap=TRUE,
        show.gene.label = FALSE,
        save.to.this.file = "R.RCandy_parC_region.pdf",
        rec.heatmap.color=c("#FF000022", "#0000FF"),
        rec.plot.bg.transparency=0.0,
        color.pallette="plasma", # plasma, cividis, viridis, magma, inferno
        #color.tree.tips.by.column="S83L",
...
for ( f in 1:length(freqs.list) ) {
    ...
    fname <- paste("R.freqs.by", trait, "Rds", sep='.')
    saveRDS(freqs, fname)
    fname <- paste("R.freqs.by", trait, "tab", sep='.')
      ...
      if (verbose) {
        ...
        Hist(freqs[ ,col], groups=freqs$ab, 
	     scale="percent", breaks=40, col="darkgray", 
             ...
             main=paste(col, "by", trait))
        # and save to a file
        png(file=paste('R.histogram-pct', trait, col, 'png', sep='.'), 
