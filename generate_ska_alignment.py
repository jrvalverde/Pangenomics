...
def map_fasta_sequence(seq, k = None, names = None):
    # vvv JR vvv
    if os.path.exists(seq + '.map.aln'):
        print('ska: skipping ' + seq)
        return
    print('ska: mapping ' + seq)
    # ^^^ JR ^^^
...
if __name__ == "__main__":
    ...
    # Generate alignment
    # XXX JR XXX	Ensure that it works
    subprocess.check_output('cat ' + ' *.map.aln' + ' > ' + args.out,
                            shell = True)
    # XXX JR XXX this fails with arg. list too long
